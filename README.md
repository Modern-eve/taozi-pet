...
- **小屋面板**：
  ...
  - **开发者模式（隐藏调试入口）**：在状态页末尾的**版本号**上 2 秒内**连点 6 次**进入（第 4、5 下会弹出倒计时气泡，第 6 下弹出「已进入开发者模式」并解锁「🧪 开发者」页签）。开发者页提供：**关闭开发者模式开关**、**💊 喂安眠药**（立即触发睡觉、遵守打断逻辑）、**🍃 飞叶子**（心情置 100）、**👊 轻抚脸颊**（心情置 0）。开启状态持久化到 `settings.json`（`devMode`），重启仍保持；关闭后可再次连点版本号进入。
