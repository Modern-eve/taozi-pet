...
```
assets-raw/  (纯白底 PNG)
   │  preprocess-v7 -gpu.py  (GPU 抠白底 → 透明)
   ▼
assets-processed/  (透明抠图)
   │  assemble-incoming-assets.py  (组装 → 按状态收进 incoming)
   ▼
taozi-pet/incoming-assets/  (未归一化的中间帧)
   │  normalize-incoming-occupancy.py  (占用率归一化 + 居中)
   ▼
taozi-pet/incoming-assets/  (归一化后)
   │  node tools/process-assets.mjs  (渲染为最终桌宠素材)
   ▼
taozi-pet/src/assets/pet/  (运行时素材)
...
| 脚本 | 作用 |
|------|------|
| `preprocess-v7 -gpu.py` | GPU 抠白底（BiRefNet + CUDA），`assets-raw/` → `assets-processed/`。CPU 回退版：`preprocess-v7-cpu.py` |
| `assemble-incoming-assets.py` | 组装 `incoming-assets`：`--matted-states`（默认 walk/peek/sleep/sad）从 `assets-processed` 渲染，其余从 `incoming-assets` 复制 |
| `normalize-incoming-occupancy.py` | 占用率归一化（默认 TARGET_OCC=0.62）+ 居中 + 底部对齐到 1280 画布 |
...
C:\PYTHON312\python.exe normalize-incoming-occupancy.py
cd taozi-pet
...
- **GPU 抠图只适配 4 个状态**（walk/peek/sleep/sad）；idle/blink/happy/notify/pet-head/pumpkin-bag/petal-spin/starfish-wave 这 8 个状态必须用 CPU(v7) incoming，否则破 QA 契约（idle/blink 的 lockedBody 一致性、其余 6 个的占用率上限）。
- **normalize 默认绝不可含 idle/blink**：二者为 lockedBody，已提交的 CPU 帧天然满足 ≤2.5% 宽高一致，归一化反而会触发 `SCALE_DRIFT`。
