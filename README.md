...
assets-processed/  (透明抠图)
   │  assemble-incoming-assets.py  (组装 + 占用率归一化 + 居中，一步完成)
   ▼
taozi-pet/incoming-assets/  (整合+归一化后的中间帧)
...
| `assemble-incoming-assets.py` | 组装 + 归一化 `incoming-assets`：`--matted-states`（默认 walk/peek/sleep/sad）从 `assets-processed` 渲染并按占用率归一化（居中 + 底部对齐到 `sourceCanvas` 画布），其余从 `incoming-assets` 复制。阈值唯一权威来自 `pet-spec.json` 的 `assetPipeline.source*`（`sourceCanvas`/`sourceMargin`/`sourceOccupancy`/`sourcePad`） |
