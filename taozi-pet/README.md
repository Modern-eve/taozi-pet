...
| `preprocess_common.py` | 被 `preprocess-v7-*.py` 与 `make-graphics.py` 导入 | 抠图管线公共实现（非独立脚本）：源/产物路径、帧名工具（`state_of` / `out_name`）、参数与清单组装（`select_frames` / `job_list`）、保护色 `get_protected_mask`、连通域后处理 `refine_alpha`、洪水填充 `flood_fill_from_edges`、贴边清理 `clear_outer_border`。**GPU 与 CPU 两条路径共用同一份阈值与判据**，调整只改这一处 |
| `preprocess-v7-gpu.py` | `assets-raw/` → `incoming-assets/` | GPU 抠白底（BiRefNet + CUDA），**默认全量**。输入 png/jpg/jpeg/webp/bmp（按内容解码，扩展名不可信），**输出统一为透明 png**；`--states` 可限定部分状态，`--cpu` 强制 CPU 推理，`--serial` 关闭三阶段流水线（默认开启）。`INFER_FP16=0` 可让推理回到 fp32。后处理见 `preprocess_common.py` |
...
- **抠图后处理的单一真源**：`preprocess_common.py` 是源/产物路径、帧名工具、保护色、连通域保留判据、洪水填充与贴边宽度的唯一实现，`preprocess-v7-gpu.py`、`preprocess-v7-cpu.py`、`make-graphics.py` 三方共用，调整只改这一处。保护色种子限定在 `alpha > 16` 内，只对模型已判为前景的像素生效，不向轮廓外扩张。
...
- **抠图耗时**（RTX 4060 Laptop）：瓶颈依次是 GPU 推理 → 连通域分析 → PNG 编码，故推理走 fp16 autocast、连通域合并为一次分析且只对候选块做全图归约、PNG 用 `compress_level=1`，并让「预取 → 推理 → 收尾」三阶段流水线并行。稳态实测 **0.39 s/帧**（主线程只剩推理时间，GPU 利用率约 82%、CPU 并行度 1.4 核）；`--serial` 关闭流水线可对照，此时 0.72 s/帧、GPU 约 53%。全量 133 帧含模型加载 97 s。**瓶颈已完全落在 GPU 推理上**——再叠加 CPU 并行、批处理或 `cudnn.benchmark` 都没有收益，再快只能动模型或输入分辨率。
